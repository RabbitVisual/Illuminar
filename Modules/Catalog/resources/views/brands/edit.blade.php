<x-core::layouts.master heading="Editar marca">
    <div class="max-w-2xl space-y-6">
        ...
        <div class="p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <form method="POST" action="{{ route('catalog.brands.update', $brand) }}">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="logo" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center gap-1">
                            Logo (URL)
                            <span data-tooltip-target="tooltip-logo-edit" class="cursor-help">
                                <x-icon name="circle-info" style="duotone" class="w-4 h-4 text-gray-400" />
                            </span>
                        </label>
                        <div id="tooltip-logo-edit" role="tooltip" class="absolute z-50 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
